...
class TransformerEncoder(Module):
    ...
    def forward(
        ...
        for mod in self.layers:
            output, attn_weights = mod(output, src_mask=mask, is_causal=is_causal, src_key_padding_mask=src_key_padding_mask_for_layers)
...
class TransformerEncoderLayer(Module):
    ...
    def forward(
        ...
        if self.norm_first:
            ...
            residual = x
            x, weights = self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, is_causal=is_causal)
            x = residual + x
            x = x + self._ff_block(self.norm2(x))
        else:
            ...
            residual = x
            x, weights = self._sa_block(x, src_mask, src_key_padding_mask, is_causal=is_causal)
